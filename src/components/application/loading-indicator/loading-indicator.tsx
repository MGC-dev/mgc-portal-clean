...
export const LoadingIndicator = ({ type = "line-simple", size = "sm", label }: LoadingIndicatorProps) => {
    const renderSpinner = () => {
        ...
        if (type === "dot-circle") {
            return (
                <svg className={cx("animate-spin text-[#264f5e]", styles[size].spinner)} viewBox="0 0 36 36" fill="none">
                    <path
        ...
        return (
            <svg className={cx("animate-spin", styles[size].spinner)} viewBox="0 0 32 32" fill="none">
                <circle className="text-black/10" cx="16" cy="16" r="14" stroke="currentColor" strokeWidth="4" />
                <circle
                    className="stroke-[#264f5e]"
                    cx="16"
                    cy="16"
    ...
    return (
        <div className={cx("flex flex-col items-center justify-center", styles[size].root)}>
            {renderSpinner()}
            {label && <span className={cx("text-gray-500", styles[size].label)}>{label}</span>}
        </div>
    );
};
